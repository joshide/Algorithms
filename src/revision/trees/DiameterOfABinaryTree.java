...
public class DiameterOfABinaryTree {
	...
	private static int diameter(TreeNode node) {
		...
		int crossOverDiameter = crossOverDiameter(node);
		
		if(leftDiameter >= rightDiameter && leftDiameter >= crossOverDiameter)
			return leftDiameter;
		else if (rightDiameter >= leftDiameter && rightDiameter >= crossOverDiameter)
			return rightDiameter;
		else
			return crossOverDiameter;
	}
	
	private static int crossOverDiameter(TreeNode node) {
		if(node == null) {
			return 0;
		}
